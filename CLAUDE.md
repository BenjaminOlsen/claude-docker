...
- **Prompt file + launcher script**: the hook writes the prompt to `.claude-prompt` and creates `.claude-launcher.sh` to avoid shell quoting issues across tmux → script → claude. Both are added to `.git/info/exclude` so Claude won't commit them.
...
- **Work branches**: Claude works on a `claude/<branch>` branch (not the user's branch directly), so the user can pull and merge at their discretion.
- **tmux sessions** are named `claude-<repo>-<branch>` with non-alphanumeric chars replaced by underscores. `tmux kill-server` is called before each new session to clean up stale sockets.
- **Docker-in-Docker**: the Docker socket is mounted into the container so Claude can run docker/compose for project tests.
